...
_log_queue: queue.Queue = queue.Queue() # pyright: ignore[reportMissingTypeArgument, reportUnknownVariableType]
...
# _CALLBACK_TYPE(...) 返回的 C 可调用对象类型别名（Pylance 无法推断 CFUNCTYPE 实例类型）
_CCallbackType = ctypes._CFuncPtr
...
class TTHSDownloader:
    ...
    def _make_c_callback(
        self,
        user_callback: Callable[[dict[str, int | float | str], dict[str, int | float | str]], None],
    ) -> _CCallbackType:
        """
